	...
	Command.AddCommand(
		&cobra.Command{
			...
			Run: func(cmd *cobra.Command, args []string) {
				...
				libraries := make([]string, 0, len(args))
				for i := range args {
					if !strings.HasPrefix(args[i], "nixpkgs/") {
						libraries = append(libraries, "nixpkgs/"+args[i])
					}
					libraries = append(libraries, args[i])
				}

				c := exec.Command("sh", "-c", fmt.Sprintf("nix shell %s --command echo downloaded", strings.Join(libraries, " ")))
